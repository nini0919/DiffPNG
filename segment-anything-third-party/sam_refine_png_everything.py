    ...
    for j in range(len(valid_phrase)):
        for k in range(len(selected_nouns_clip_idx[j])):
            ...
            for t in range(masks.shape[0]):
                _foreground = masks[t]
                # inter_1 = (_foreground * c[b]).sum()/(_foreground.sum())
                # inter_2 = (_foreground * c[b]).sum()/(c[b].sum()+1e-9)
                inter = (_foreground * pseudo_part).sum()/(_foreground+pseudo_part-_foreground * pseudo_part).sum()
                if inter > 0.3:
                    if inter > inter_top1:
                        inter_top1 = inter
                    ...
                    cnt +=1
            if cnt ==masks.shape[0]:
                refine_masks = pseudo_part
            else:
                refine_masks[foreground.bool()] = 1
            plt.subplot(141)
